...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],current_to_obs=0):
    ...
    if (avg_neighbors_to_obs[next_node_idx] > middle_value_neighbors_to_obs):
        ...
        reward += distance*2
        if distance >= 0:
            reward += (middle_value_neighbors_to_obs - ranking_neighbors_distance_to_obs[next_node_idx])*30
        else:
            reward -= (middle_value_neighbors_to_obs - ranking_neighbors_distance_to_obs[next_node_idx])*20   
