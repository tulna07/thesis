...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],current_to_obs=0):
    ...
    if (ranking_neighbors_distance_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
        ...
        # reward += distance
        if distance >= 0:
            reward += (len(ranking_neighbors_distance_to_obs) - ranking_neighbors_distance_to_obs[next_node_idx])*50 + distance*3
        else:
            reward -= ranking_neighbors_distance_to_obs[next_node_idx]*50 - distance*3   
