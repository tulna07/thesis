...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table, obs_ls,view_map=False):
    ...
    if not view_map:              
        ...
        for index in range(len(neighbor_nodes)):
            if visited_neighbor_nodes[robot_action_idx].coords == neighbor_nodes[index].coords:
                q_table[robot_state][index] = new_q*0.3
                break
        
    return robot_state, robot_arobot_next_statection_idx
...
def train(start, goal, obstacles=Obstacles(), vision_range=5, Tree=Tree, view_map=False):
    ...
    for episode in range(HM_EPISODES):
        ...
        while True:
            ...
            robot_state, robot_action_idx = run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles,q_table,obs_ls,view_map)
