...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[], current_to_obs=0):
    ...
    if next_node.checkin:
        ...
        if distance >= 0:
            ...
            if (distance_obs < 0):
                reward -=  abs(distance_obs)*20 #ranking_neighbors_distance_to_obs[next_node_idx]*50     
            else:
                reward += distance_obs*40 #(len(ranking_neighbors_distance_to_obs) - ranking_neighbors_distance_to_obs[next_node_idx])*50        
        else:
