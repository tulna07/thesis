...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],=0):
    reward = 0
    ...
    if (avg_neighbors_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
        if distance >= 0:
            reward += (len(ranking_neighbors_distance_to_obs) - avg_neighbors_to_obs[next_node_idx])*10
        else:    
            reward -= avg_neighbors_to_obs[next_node_idx]*50     
    else:
        reward += distance
        if distance >= 0:
            reward += (len(ranking_neighbors_distance_to_obs) - avg_neighbors_to_obs[next_node_idx])*50
        else:
            reward -= avg_neighbors_to_obs[next_node_idx]*50  
