...
def highest_value_index(arr =[]):
    highest_value =  0
    highest_idx = 0
    for idx in range(len(arr)):
        if highest_value == 0 or highest_value < arr[idx][0]:
            highest_value = arr[idx][0]
            highest_idx = idx
    return highest_idx        
...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table, obs_ls):
    ...
    if not robot_state in q_table:
        q_table[robot_state] = [[0,0] for i in range(len(visited_neighbor_nodes))]
    
    #take move base on highest q-value
    idx = rng.integers(0,len(uniform_float_arr))
    robot_action_idx = highest_value_index(q_table[robot_state])
    if uniform_float_arr[idx] > epsilon or q_table[robot_state][robot_action_idx][0] >= 1000: 
    ...
    # update q table 
    q_table[robot_state][robot_action_idx][1] += 1
    ...
    if not robot_next_state in q_table:
        q_table[robot_next_state] = [[0,0] for i in range(len(next_visited_neighbor_nodes))]
    # max_future_q = np.max(q_table[robot_next_state])
    max_future_q = q_table[robot_next_state][highest_value_index(q_table[robot_next_state])][0]
    current_q = q_table[robot_state][robot_action_idx][0]
    if reward == GOAL_REWARD:
        new_q = GOAL_REWARD
    else:
        new_q = (1 - LEARNING_RATE) * current_q + LEARNING_RATE * (reward + DISCOUNT * max_future_q)
        q_table[robot_state][robot_action_idx][0] = new_q
...
def train(start, goal, obstacles=Obstacles(), vision_range=5, Tree=Tree, view_map=False):
    ...
    for episode in range(HM_EPISODES):
        ...
        print("episode:", episode+1, ", total nodes:", len(Tree.path_to_goal), ", total path length:", total_path_length)        
...
if __name__ == '__main__':
    ...
    if not read_tree:
        ...
        for idx in range(len(input_array)):
            epsilon = 0.9
            start_cooridinate = input_array[idx]
