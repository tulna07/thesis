...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],check=Boolean,next_node_nearest_ls=[]):
    reward = 0
    next_node_idx = get_node_index(next_node,visited_neighbor_nodes)
    current_node_to_further_ls = get_further_point_line_segment(current_node, next_node_nearest_ls)
    next_node_to_further_ls = get_further_point_line_segment(next_node, next_node_nearest_ls)
