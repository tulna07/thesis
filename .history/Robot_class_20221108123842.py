...
class Robot(Robot_base):
    ...
    def avg_distance_to_obs(self, neighbor_nodes, obs_ls):
        avg_neighbors_to_obs = []
        temp_distance = 0
        for idx in range(len(neighbor_nodes)):
            for ls in obs_ls:
                temp_distance += point_dist(ls[0],neighbor_nodes[idx].coords)
            avg_neighbors_to_obs.append(temp_distance)
        avg_node_to_obs = np.array(avg_node_to_obs)/len(obs_ls)
        return avg_node_to_obs