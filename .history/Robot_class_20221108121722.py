...
class Robot(Robot_base):
    ...
    def avg_distance_to_obs(self, current_node, next_node, obstacles):
        avg_node_to_obs = 0
        check, obs_ls = self.check_intersection_obs(obstacles,current_node,next_node)
        for ls in obs_ls:
            avg_node_to_obs += point_dist(ls[0],current_node.coords)
            
        avg_node_to_obs = avg_node_to_obs/len(obs_ls)
