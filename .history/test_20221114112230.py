...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],check=Boolean,nearest_ls=[]):
    ...
    if next_node.checkin:
        ...
        if not check and next_node_to_further_ls >= current_node_to_further_ls and point_dist(current_node.coords,Tree.root.coords) > point_dist(next_node.coords,Tree.root.coords):
            reward += 30    
    return reward
