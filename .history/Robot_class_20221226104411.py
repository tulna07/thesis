...
class Robot(Robot_base):
    ...
    def angle_waypoints(self,path_to_goal):
        waypoints_angle = []
        for idx in range(len(path_to_goal)-2):
            angle = self.angle_between(path_to_goal[idx].coords,path_to_goal[idx+1].coords,path_to_goal[idx+2].coords)
            waypoints_angle.append(angle)
        return waypoints_angle    
