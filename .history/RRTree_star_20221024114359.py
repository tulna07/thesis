...
def robot_main(start, goal, obstacles=Obstacles(), vision_range=5, Tree=Tree):
    ...
    for episode in range(HM_EPISODES):
        ...
        Tree.path_to_goal = []
        while True:
            state = robot.get_robot_coords()
            # visualize
            node = Tree.get_node_by_coords(state)
            Tree.path_to_goal.append(node)
            
            if not state in q_table:
                q_table[state] = [np.random.uniform(-5, 0) for i in range(80)]
            ...
            next_state = robot.get_robot_coords()
            if not next_state in q_table:
                q_table[next_state] = [np.random.uniform(-5, 0) for i in range(80)]
            max_future_q = np.max(q_table[next_state])
            current_q = q_table[next_state][action]
