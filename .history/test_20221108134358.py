...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[], cur_to_obs=[]):
    reward = 0
    ...
    # forth condition  
    if avg_neighbors_to_obs[next_node_idx] >= cur_to_obs[0]:    
...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table, obs_ls):
    ...
    cur_to_obs = robot.avg_neighbors_distance_to_obs(arr,obs_ls)
    ...
    if robot.reach_goal:
        ...
        reward = evaluate_reward(Tree, current_node, next_node, visited_neighbor_nodes, avg_neighbors_to_obs, cur_to_obs)
