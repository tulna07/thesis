...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],check=Boolean,ls=[]):
    reward = 0
    next_node_idx = get_node_index(next_node,visited_neighbor_nodes)
    current_node_to_further_ls = get_further_point_line_segment(current_node, ls)
    next_node_to_further_ls = get_further_point_line_segment(next_node, ls)
    ...
    if next_node.checkin:
        ...
        if (ranking_neighbors[next_node_idx] >= middle_value_neighbors):
            reward -= (ranking_neighbors[next_node_idx] - middle_value_neighbors)*20 
        else:
            reward += (middle_value_neighbors - ranking_neighbors[next_node_idx])*20     
        ...
        if not check_and_ls[0] and next_node_to_further_ls >= current_node_to_further_ls:
            reward += 30    
    return reward
...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table, obs_ls):
    ...
    if robot.reach_goal:
        ...
        check , obs = robot.check_intersection_obs(obstacles, next_node_path[0],next_node_path[1])
        nearest_ls = robot.nearest_line_segment(next_node_path[0], obs)
        reward = evaluate_reward(Tree, current_node, next_node, visited_neighbor_nodes, avg_neighbors_to_obs,check,lnearest_lss)
