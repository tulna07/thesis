...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],current_to_root=z):
    reward = 0
...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table,temp_q_table, obs_ls,view_map=False,randomness=int):
    ...
    if not view_map:   
        ...
        if robot.reach_goal:
            ...
            current_to_root = robot.cal_distance_to_line_segment(current_node,robot.nearest_line_segment(current_node,obs_ls))
            reward = evaluate_reward(Tree, current_node, next_node, visited_neighbor_nodes, avg_neighbors_to_obs, current_to_root)
