...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],check =Boolean):
    ...
    if next_node.checkin:
        ...
        if (ranking_neighbors_distance_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
            ...
            # reward += distance*3
            distance = Tree.distances(current_node.coords,visited_neighbor_nodes)
            ranking_neighbors_to_cur = ranking_list(distance)
            middle_neighbors_to_cur = middle_value_in_list(distance)
            if (ranking_neighbors_to_cur[next_node_idx] >= middle_neighbors_to_cur):
                reward += ranking_neighbors_to_cur[next_node_idx]*50  
            else:
                reward -= (len(ranking_neighbors_to_cur) - ranking_neighbors_to_cur[next_node_idx])*10
