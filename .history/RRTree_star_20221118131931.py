...
def get_node_and_neighbors(Tree,robot,obstacles, vision_range):
    current_node = Tree.get_node_by_coords(robot.get_robot_coords())
    neighbor_nodes = Tree.neighbour_nodes(robot.get_robot_coords(), vision_range)
...
def update_q_table(q_table, temp_q_table,Tree,robot,obstacles, vision_range):
    for temp_key in temp_q_table:
            for key in q_table:
                if key == temp_key:
                    for idx in range(len(temp_q_table[temp_key])):
                        current_node, neighbor_nodes, visited_neighbor_nodes = get_node_and_neighbors() 
                        q_table[key] = temp_q_table[temp_key]
...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table,temp_q_table, obs_ls,view_map=False):
    ...
    current_node, neighbor_nodes, visited_neighbor_nodes = get_node_and_neighbors(Tree,robot,obstacles,vision_range)
    ...
    next_node, next_neighbor_nodes, next_visited_neighbor_nodes = get_node_and_neighbors(Tree,robot,obstacles,vision_range)
    if not view_map:   
