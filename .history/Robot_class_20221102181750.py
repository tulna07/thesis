...
class Robot(Robot_base):
    ...
    def check_next_neighbor_node_in_rrtstar(self, path_to_goal, neighbour_nodes):
        for node in neighbour_nodes:
            if node.coords == path_to_goal[1].coords:
                return True
        return False

    ''' check next neighbor node in vision range collides obstacles '''
    def check_path_collides_obstacles(self, line_segments, obstacles_line_segments):
        for obstracle_lss in obstacles_line_segments:
            for ls in obstracle_lss:
                    pt_is = line_across(ls, line_segments)
                    if pt_is:
                        return True
        return False
    
    # check neighbour nodes in obstacles at current position 
    def scan_obstacles(self, neighbor_nodes, obstacles, path_to_goal):
        line_segments =[]
        if neighbor_nodes is None:
            return []
        
        # check = self.check_next_neighbor_node_in_rrtstar(path_to_goal, neighbor_nodes)
        # if check:
