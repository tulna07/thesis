...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[], current_to_obs=0):
    ...
    if next_node.checkin:
        ...
        if distance >= 0:
            # reward += distance*10
            # distance_obs = current_to_obs - avg_neighbors_to_obs[next_node_idx]
            # if distance_obs < 0:
            #     reward -= abs(distance_obs)*10    
            # else:
            #     reward += distance_obs*10
            reward += point_dist(current_node.coords, next_node.coords)*10
        else:
            reward -= abs(distance)*10    
...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table, obs_ls, view_map=False):
    robot_action = 0 
    robot_action_idx = 0
    reward = 0
...
def train(start, goal, obstacles=Obstacles(), vision_range=5, Tree=Tree, view_map=False):
    ...
    for episode in range(HM_EPISODES):
        ...
        while True:
            ...
            reward = run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles,q_table,obs_ls,view_map)
            if not view_map:
