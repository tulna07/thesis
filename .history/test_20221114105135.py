...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],check_and_ls):
    reward = 0
    next_node_idx = get_node_index(next_node,visited_neighbor_nodes)
    current_node_to_further_ls = get_further_point_line_segment(current_node, check_and_ls[1])
    next_node_to_further_ls = get_further_point_line_segment(next_node, check_and_ls[1])
...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table, obs_ls):
    ...
    if robot.reach_goal:
        ...
        check_and_ls = robot.check_intersection_obs(obstacles, next_node_path[0],next_node_path[1])
        reward = evaluate_reward(Tree, current_node, next_node, visited_neighbor_nodes, avg_neighbors_to_obs,check_and_ls)
...
if __name__ == '__main__':
    ...
    if not read_tree:
        ...
        for idx in range(len(input_array)):
            epsilon = 0.9
            start_cooridinate = input_array[idx+4]
