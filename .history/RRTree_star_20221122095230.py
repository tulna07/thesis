...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],sh):
    reward = 0
...
def train(start, goal, obstacles=Obstacles(), vision_range=5, Tree=Tree, view_map=False):
    ...
    for episode in range(HM_EPISODES):
        ...
        shortest_path = get_shortest_path_length(shortest_path,path_to_goal)
        if episode%50 == 0:
