...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],obstacles):
    reward = 0
    next_node_idx = get_node_index(next_node,visited_neighbor_nodes)
    next_node_path = Tree.path_to_root(next_node)
    next_ls = [[next_node_path[0].coords,next_node_path[1].coords]]
    # first condition
...
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table, obs_ls):
    ...
    if robot.reach_goal:
        ...
        reward = evaluate_reward(Tree, current_node, next_node, visited_neighbor_nodes, avg_neighbors_to_obs,obstacles)
