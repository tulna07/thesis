...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[]):
    ...
    if next_node.checkin:
        ...
        if distance >= 0:
            ...
            if (ranking_neighbors_distance_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
                reward -= ranking_neighbors_disimport numpy as np
import pickle
import sys
...
def run_by_reinforcement_learning(view_map,goal, vision_range, robot, Tree, obstacles, q_table, obs_ls):
    robot_action = 0 
    robot_action_idx = 0
    reward = 0
    avg_neighbors_to_obs = []
...
def train(start, goal, obstacles=Obstacles(), vision_range=5, Tree=Tree, view_map=False):
    ...
    for episode in range(HM_EPISODES):
        ...
        while True:
            ...
            reward = run_by_reinforcement_learning(view_map,goal, vision_range, robot, Tree, obstacles,q_table,obs_ls)
            if not view_map:
                handle_q_table(save_q_table, q_table)
            episode_reward += reward
