...
def highest_index(arr=[]):
    ...
    for idx in range(len(arr)):
        if highest_q_value == 0 or highest_q_value < arr[idx][0]:
            highest_q_value = arr[idx][0]
            highest_idx = idx
    return highest_idx    
def run_by_reinforcement_learning(goal, vision_range, robot, Tree, obstacles, q_table, obs_ls):
    ...
    robot_action_idx = highest_index(q_table[robot_state])
    idx = rng.integers(0,len(uniform_float_arr))
    if uniform_float_arr[idx] > epsilon or q_table[robot_state][robot_action_idx][1] >= 100: 
    ...
    max_future_q = highest_index(q_table[robot_next_state])
