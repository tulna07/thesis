...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[]):
    ...
    if (avg_neighbors_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
        if distance >= 0:
            reward += (middle_value_neighbors_to_obs/avg_neighbors_to_obs[next_node_idx])*100
        else:    
            reward -= avg_neighbors_to_obs[next_node_idx]*50     
    else:
        reward += distance
        if distance >= 0:
            reward += (middle_value_neighbors_to_obs - avg_neighbors_to_obs[next_node_idx])*500
        else:
            reward -= avg_neighbors_to_obs[next_node_idx]*50  
