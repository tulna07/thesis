...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[]):
    ...
    for neighbor_node in visited_neighbor_nodes:
        neighbor_to_root.append(get_total_path_length(Tree.path_to_root(neighbor_node)))
    # next_to_root = get_total_path_length(Tree.path_to_root(next_node))
    middle_value_neighbors_root = middle_value_in_list(neighbor_to_root)
    
    # if (ranking_neighbors_distance_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
    #     reward -= ranking_neighbors_distance_to_obs[next_node_idx]*20     
    # else:
    #     reward += middle_value_neighbors_root - ranking_neighbors_root[next_node_idx]*10
    #     if (ranking_neighbors_root[next_node_idx] < middle_value_neighbors_root):
    ...
    if (neighbor_to_root[next_node_idx] >= middle_value_neighbors_root):
        reward -= neighbor_to_root[next_node_idx]*20
    else:
        if (avg_neighbors_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
            reward -= avg_neighbors_to_obs[next_node_idx]*10
        else:
            reward += middle_value_neighbors_to_obs - avg_neighbors_to_obs[next_node_idx]    
