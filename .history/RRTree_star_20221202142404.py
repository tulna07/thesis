...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[]):
    ...
    if (avg_neighbors_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
        if distance >= 0:
            reward += (middle_value_neighbors_to_obs/avg_neighbors_to_obs[next_node_idx])*0.8
                        reward += distance*0.01
        else:    
            reward -= avg_neighbors_to_obs[next_node_idx]
   
    else:
        if distance >= 0:
            reward += (middle_value_neighbors_to_obs/avg_neighbors_to_obs[next_node_idx]) 
        else:
            reward -= avg_neighbors_to_obs[next_node_idx]*0.8
    ...
    #     if distance >= 0:
    #         reward += (len(ranking_neighbors_distance_to_obs) - ranking_neighbors_distance_to_obs[next_node_idx])*0.9
    #     else:    
    #         reward -= ranking_neighbors_distance_to_obs[next_node_idx] + distance*0.05    
    # else:
    #     if distance >= 0:
    #         reward += (len(ranking_neighbors_distance_to_obs) - ranking_neighbors_distance_to_obs[next_node_idx])
    #     else:
    #         reward -= ranking_neighbors_distance_to_obs[next_node_idx]*0.8 + distance*0.05
...
if __name__ == '__main__':
    ...
    if not read_tree:
        ...
        for idx in range(len(input_array)):
            epsilon = 0.9
            start_cooridinate = input_array[idx+9]
            ...
            if view_map:
                ...
                print("Finish training node:", start_cooridinate) 
