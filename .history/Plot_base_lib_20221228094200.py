...
class Plot_base:
    ...
    ''' prepare title for display'''
    def prepare_title_RRT_star(self, tree):
        plot_title= ""
        if tree.total_goal_cost > 0 and tree.total_goal_cost != float('inf') and tree.path_to_goal[len(tree.path_to_goal) -1].coords == tree.root.coords :
