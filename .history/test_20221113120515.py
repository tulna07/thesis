...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[],check_next_node=bool):
    reward = 0
    ...
    if next_node.checkin:
        ...
        if not check_next_node:
            ...
            if (ranking_neighbors[next_node_idx] >= middle_value_neighbors):
                reward -= (ranking_neighbors[next_node_idx] - middle_value_neighbors)*30 
            else:
                reward += (middle_value_neighbors - ranking_neighbors[next_node_idx])*30     
        ...
        if (ranking_neighbors_distance_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
            reward -= ranking_neighbors_distance_to_obs[next_node_idx]*40     
        else:
            reward += (len(ranking_neighbors_distance_to_obs) - ranking_neighbors_distance_to_obs[next_node_idx])*40 
