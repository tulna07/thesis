...
class RRTree_star(RRTree):
    ...
    def draw_RRT_star(self,  goal_coordinate, start_coordinate, plotter: Plot_RRT=None, obstacles=None):
        plotter.build_RRT_star(Tree=self, obstacles=obstacles, goal_coords=goal_coordinate, \
                        ...
                        for idx in range(len(neighbor_nodes)):
                            if visited_neighbor_nodes[index].coords == neighbor_nodes[idx].coords:
                                if q_table[key][idx] == 0:
                                    ...
                                    q_table[key][idx] = temp_q_table[temp_key][index]*0.4 + q_table[key][idx]*0.6
...
def evaluate_reward(Tree = Tree, current_node = Node, next_node = Node , visited_neighbor_nodes=[], avg_neighbors_to_obs=[]):
    ...
    if (avg_neighbors_to_obs[next_node_idx] >= middle_value_neighbors_to_obs):
        if distance >= 0:
            reward += (middle_value_neighbors_to_obs/avg_neighbors_to_obs[next_node_idx])*0.1
        else:    
        ...
        if distance >= 0:
            ...
            # reward -= avg_neighbors_to_obs[next_node_idx]*2
            reward += (middle_value_neighbors_to_obs/avg_neighbors_to_obs[next_node_idx])*0.5
...
def train(start, goal, obstacles=Obstacles(), vision_range=5, Tree=Tree, view_map=False):
    ...
    for episode in range(HM_EPISODES):
        ...
        if episode%50==0:
            ...
            if goal_reached:
                print("Episode:", episode+1,", node:", start, ", total path length:", Tree.total_goal_cost, ", waypoints:", len(waypoints))
