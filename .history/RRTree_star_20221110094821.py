...
def choose_node(node = Node, Tree = Tree):
    input_coords = []
    nearest_node = 0
    nodes_coords = Tree.all_nodes_coordinate
    for node_coords in nodes_coords:
        temp_coords = point_dist(node.coords, node_coords)
        if nearest_node == 0 or nearest_node >= temp_coords:
            nearest_node = temp_coords
...
if __name__ == '__main__':
    ...
    if read_tree:
        start_cooridinate = choose_node(start_cooridinate, RRT_star)
        goal_coordinate = choose_node(goal_coordinate, RRT_star)
